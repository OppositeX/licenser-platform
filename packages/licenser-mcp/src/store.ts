...
export function store(): SupabaseClient {
  if (_client) return _client;
  const url =
    process.env.LICENSER_SUPABASE_URL ||
    process.env.NEXT_PUBLIC_SUPABASE_URL ||
    '';
  const key =
    process.env.LICENSER_SUPABASE_SERVICE_ROLE_KEY ||
    process.env.SUPABASE_SERVICE_ROLE_KEY ||
    '';
  if (!url || !key) {
    throw new Error(
      'Missing Supabase config. Set LICENSER_SUPABASE_URL and LICENSER_SUPABASE_SERVICE_ROLE_KEY (or NEXT_PUBLIC_SUPABASE_URL / SUPABASE_SERVICE_ROLE_KEY).',
    );
  }
...
export function generateLicenseKey(prefix = 'LIC'): string {
  const seg = () =>
    crypto.randomBytes(5).toString('base64').replace(/[^A-Z0-9]/gi, '').toUpperCase().slice(0, 4).padEnd(4, 'X');
...
export async function listLicenses(args: {
  ...
  if (args.email) q = q.ilike('customer_email', args.email);
  if (args.productSlug) {
...
export async function getLicense(args: { key?: string; id?: string }) {
  let q = store().from('licenses').select('*, products(slug,name), plans(slug,name,max_activations)');
  if (args.id) q = q.eq('id', args.id);
  else if (args.key) q = q.eq('key', args.key);
  else throw new Error('Provide key or id');
  ...
  const { data: activations } = await store()
    ...
    .eq('license_id', (license as { id: string }).id)
...
export async function issueLicense(args: {
  ...
  if (!product) throw new Error(`Unknown product slug "${args.productSlug}"`);
  ...
  if (args.planSlug) {
    const { data: plan } = await store()
      ...
      .maybeSingle();
    if (!plan) throw new Error(`Unknown plan slug "${args.planSlug}" for product "${args.productSlug}"`);
    planId = (plan as { id: string }).id;
    planMax = (plan as { max_activations: number }).max_activations;
  }
  ...
  await store().from('events').insert({
    type: 'license.issued', license_id: (data as { id: string }).id, product_id: product.id,
    data: { plan_slug: args.planSlug ?? null, by: 'mcp' },
  });
  return data; // includes the plaintext key — surface it once to the operator.
}

export async function setLicenseStatus(args: { key?: string; id?: string; status: LicenseStatus }) {
  const target = args.id ? { col: 'id', val: args.id } : { col: 'key', val: args.key };
  if (!target.val) throw new Error('Provide key or id');
  const { data, error } = await store()
    .from('licenses')
  ...
  await store().from('events').insert({
    type: `license.${args.status}`, license_id: (data as { id: string }).id, data: { by: 'mcp' },
  });
  return data;
}

export async function listActivations(args: { licenseId?: string; key?: string }) {
  ...
  if (!licenseId && args.key) {
    ...
    licenseId = (data as { id: string } | null)?.id;
  }
  if (!licenseId) throw new Error('Provide licenseId or key');
  const { data, error } = await store()
    .from('activations')
...
export async function deactivateActivation(args: { activationId?: string; key?: string; domain?: string }) {
  ...
  if (!id && args.key && args.domain) {
    ...
    const { data: act } = await store()
      .from('activations').select('id').eq('license_id', (lic as { id: string }).id)
      .eq('site_url', args.domain.toLowerCase().replace(/^https?:\/\//, '').replace(/^www\./, '').replace(/\/.*$/, ''))
      .maybeSingle();
    id = (act as { id: string } | null)?.id;
  }
  if (!id) throw new Error('Provide activationId, or key + domain');
  const { data, error } = await store()
    .from('activations').update({ status: 'deactivated' }).eq('id', id).select('id,site_url,status').maybeSingle();
  if (error) throw error;
  if (!data) throw new Error('Activation not found');
  return data;
}
...
export async function createProduct(args: { slug: string; name: string; githubRepo?: string }) {
  const { data, error } = await store()
    .from('products')
...
export async function listPlans(args: { productSlug?: string }) {
...
export async function listReleases(args: { productSlug?: string }) {
  let productId: string | undefined;
  if (args.productSlug) {
    const p = await productBySlug(args.productSlug);
    if (!p) return [];
...
export async function validationStats(days = 7) {
  ...
  for (const r of (data ?? []) as Array<{ result: string }>) counts[r.result] = (counts[r.result] ?? 0) + 1;
