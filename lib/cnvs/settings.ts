...
/**
 * Per-seat included credits, keyed by the tier a license actually resolves to.
 * The three tiers we SELL — starter / designer / powerhouse — are what cnvs-4's
 * `313-perseat-drift` gate diffs against this endpoint; `pro` / `studio` are the
 * legacy aliases kept so nothing that still reads them breaks. All are optional
 * on the wire (cnvs-4 falls back per-key), but the three sold tiers are required
 * of a stored row so a price can never silently vanish. No normalizeTier remap.
 */
export interface CnvsPerSeat {
  starter: number;
  designer: number;
  powerhouse: number;
  pro?: number;
...
/** The perSeat tiers this endpoint knows how to serve (order is cosmetic). */
export const CNVS_PERSEAT_TIERS = ['starter', 'designer', 'powerhouse', 'pro', 'studio'] as const;
/** The tiers a stored perSeat row must carry — the ones we actually sell. */
export const CNVS_PERSEAT_REQUIRED = ['starter', 'designer', 'powerhouse'] as const;
...
export const CNVS_DEFAULTS: CnvsSettings = {
  ...
  credits: { perSeat: { starter: 500, designer: 1500, powerhouse: 4000, pro: 500, studio: 1000 }, freeDaily: 20, rolloverMonths: 1 },
  rateCard: [
...
export const CNVS_VALIDATORS: Record<CnvsSection, (value: unknown) => string[]> = {
  ...
  credits(value) {
    ...
    if (!isPlainObject(perSeat)) {
      ...
      // The three sold tiers must be present and valid — those are the numbers
      // cnvs-4 prices real licenses with and diffs in its drift gate.
      for (const tier of CNVS_PERSEAT_REQUIRED) {
        if (!isNonNegNumber(perSeat[tier])) errs.push(`credits.perSeat.${tier}: must be a non-negative number`);
      }
      // Legacy aliases are optional, but must be valid when present.
      for (const tier of ['pro', 'studio'] as const) {
        if (perSeat[tier] !== undefined && !isNonNegNumber(perSeat[tier])) {
          errs.push(`credits.perSeat.${tier}: must be a non-negative number when present`);
        }
      }
    }
...
function normalizeSection(section: CnvsSection, raw: unknown): unknown {
  ...
  switch (section) {
    ...
    case 'credits': {
      // Preserve every known tier that is present (sold tiers + legacy
      // aliases), strip anything else, so the stored numbers reach cnvs-4.
      const src = (o.perSeat ?? {}) as Record<string, unknown>;
      const perSeat: Record<string, number> = {};
      for (const tier of CNVS_PERSEAT_TIERS) {
        if (typeof src[tier] === 'number') perSeat[tier] = src[tier] as number;
      }
      return {
        perSeat,
